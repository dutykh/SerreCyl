...
\begin{align}
  ...
  & \bigl(\,1\,-\,\tilde{\alpha}\,r_{\,0,\,xx}\,\bigr)\,u_{\,t}\,+\,\bigl(\,\tilde{\beta}\,\eta\,\bigr)_{\,x}\,+\,u\,u_{\,x}\,-\,\frac{(\,4\,\tilde{\alpha}\,+\,r_{\,0}\,)\,r_{\,0}}{8}\,u_{\,xxt}\,+\,\frac{(\,3\,\tilde{\alpha}\,+\,r_{\,0}\,)\,r_{\,0,\,x}}{2}\,\bigl(\,\tilde{\beta}\,\eta\,\bigr)_{\,xx}\,+\,\kappa\,u\,-\,\gamma\,\Bigl[\,\tilde{\beta}\,\bigl(\,r_{\,0,\,x}\,u\,+\,\tfrac{1}{2}\,r_{\,0}\,u_{\,x}\,\bigr)\,\Bigr]_{\,x}\,=\,0\,, \label{eq:MDLP2}
\end{align}
...
\end{equation}
the divergence identity behind which is verified symbolically in \texttt{scripts/verify\_energy.py}. Substituting the slowly-varying profile $\eta=A(t)\sech^{2}(K\zeta)$ with the width--amplitude relation $K^{2}=10A/[r_{0}^{2}(12\tilde{\alpha}+3r_{0})]$ and the integrals $\int\eta^{2}=\tfrac{4A^{2}}{3K}$, $\int\eta_{x}^{2}=\tfrac{16A^{2}K}{15}$ into \eqref{eq:L2balance} and simplifying gives \eqref{eq:decay_law} (the elementary algebra is checked in \texttt{scripts/verify\_soliton\_decay.py}). The excess mass is radiated into a small trailing shelf, consistent with the exact conservation of $\int\eta\,\ud x$.
\end{proof}
